...
exports.login = async (req, res) => {
  try {
    const { email, password, userGoogleId, userName } = req.body;
    ...
    if (userGoogleId) {
      ...
      if (!user) {
        ...
        const userFound = await User.findOne({ userGoogleId });
        const token = createAccessToken({ id: userFound._id });
        delete userFound.password;
        return res
          .status(200)
          .json({ token,  user:userFound, status: true, msg: "Login successful.." });
      }
